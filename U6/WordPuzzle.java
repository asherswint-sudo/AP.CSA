...
public class WordPuzzle {
     ...
     */
//     A box is labeled with a positive number if and only if
// The box is white and
//  The box does not have a white box immediately above it, or it does not have a white box immediately to its left, or both.
    public boolean toBeLabeled(int r, int c, boolean [][] blackBoxes){
        if(blackBoxes[r][c]) return false;
        if(r == 0 || c == 0) return true;
        if(!blackBoxes[r-1][c] && !blackBoxes[r][c-1]) return false;                       
		return true;  
    }
